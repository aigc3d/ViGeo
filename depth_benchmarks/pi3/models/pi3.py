...
class Pi3(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, imgs):
        imgs = resize_tensor_by_pixel_limit(imgs)
        if imgs.dim() == 4:
        ...
        return dict(
            ...
            depth=z.permute(0, 1, 4, 2, 3).squeeze(0).squeeze(1)
        )

def resize_tensor_by_pixel_limit(tensor: torch.Tensor, pixel_limit: int = 255000) -> torch.Tensor:
    B, C, H_orig, W_orig = tensor.shape
