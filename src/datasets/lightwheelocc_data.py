...
class lightwheelocc_data(base_data):
    def __init__(
        self,
        **kwargs
    ):
    ...
    def load_depth(self, depth_path):
        depth_img = cv2.imread(os.path.join(self.data_path, depth_path), cv2.IMREAD_UNCHANGED).astype(np.float32)
        depth = depth_img[:,:,0] + (depth_img[:,:,1] * 256)
        depth = depth * 0.01
        return depth.astype(np.float32), (depth == 0).astype(np.float32)
