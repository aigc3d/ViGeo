...
class ConvStack(nn.Module):
    ...
    def _forward_impl(self, in_features: List[torch.Tensor]) -> List[torch.Tensor]:
        out_features = []
        x = None
        for i in range(len(self.res_blocks)):
            ...
            if i == 0:
                x = feature
            elif feature is not None and x is not None:
                x = x + feature

            if x is None:
                if feature is None:
                    out_features.append(None)
            ...
            output = self.output_blocks[i](x)
            if isinstance(self.output_blocks[i], nn.Identity):
                output = output.clone()
    ...
    def forward(self, in_features: List[torch.Tensor], chunk_size: Optional[int] = 8) -> List[torch.Tensor]:
        if chunk_size is None:
            return self._forward_impl(in_features)

        if not all(f.shape[0] == in_features[0].shape[0] for f in in_features if f is not None):
             raise ValueError("All input features must have the same batch size for chunking.")

        valid_features = [f for f in in_features if f is not None]
        if not valid_features:
            return self._forward_impl(in_features)

        full_batch_size = valid_features[0].shape[0]

        in_feature_chunks = [
            list(torch.split(f, chunk_size, dim=0)) if f is not None else [None] * ((full_batch_size + chunk_size - 1) // chunk_size)
            for f in in_features
        ]

        output_chunks_per_level = [[] for _ in range(len(self.res_blocks))]

        for chunk_group in zip(*in_feature_chunks):
            chunk_output_list = self._forward_impl(list(chunk_group))

            for i, out_chunk in enumerate(chunk_output_list):
                if out_chunk is not None:
                    output_chunks_per_level[i].append(out_chunk)

        final_output = [
            torch.cat(chunks, dim=0) if chunks else None
            for chunks in output_chunks_per_level
        ]
