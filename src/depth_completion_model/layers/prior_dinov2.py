...
class PriorDinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(
        ...
        x_img,
        x_depth,
        x_depth_coarse: torch.Tensor | None = None,
        x_depth_mask: torch.Tensor | None = None,
        x_depth_coarse_mask: torch.Tensor | None = None,
    ):
        B, S, _, h_img, w_img = x_img.shape
        _, _, _, h_depth, w_depth = x_depth.shape
        BS = B * S

        if x_depth_mask is None:
            raise ValueError("x_depth_mask must be provided for explicit prior masking.")
        if x_depth_mask.shape != x_depth.shape:
            raise ValueError("x_depth_mask must have the same shape as x_depth.")

        valid_sparse_mask = x_depth_mask.to(device=x_depth.device, dtype=torch.bool)
        x_depth = torch.where(valid_sparse_mask, x_depth, torch.zeros_like(x_depth))

        if x_depth_coarse is not None and hasattr(self, 'coarse_depth_patch_embed'):
            if x_depth_coarse_mask is None:
                raise ValueError("x_depth_coarse_mask must be provided when x_depth_coarse is set.")
            if x_depth_coarse_mask.shape != x_depth_coarse.shape:
                raise ValueError("x_depth_coarse_mask must have the same shape as x_depth_coarse.")

            valid_coarse_mask = x_depth_coarse_mask.to(device=x_depth_coarse.device, dtype=torch.bool)
            x_depth_coarse = torch.where(valid_coarse_mask, x_depth_coarse, torch.zeros_like(x_depth_coarse))
            combined_valid_mask = valid_sparse_mask | valid_coarse_mask
            ...
            combined_valid_mask = valid_sparse_mask
            x_coarse_depth_tokens = 0

        x_img = x_img.reshape(BS, -1, h_img, w_img)
        x_depth = x_depth.reshape(BS, -1, h_depth, w_depth)
        valid_sparse_mask = valid_sparse_mask.reshape(BS, -1, h_depth, w_depth)
        combined_valid_mask = combined_valid_mask.reshape(BS, -1, h_depth, w_depth)
        ...
        invalid_depth_patch_mask = _compute_depth_invalid_mask(
            depth_values=valid_sparse_mask.float(),
            H_patch=patch_h,
            W_patch=patch_w,
        ...
        x_depth_masked, _ = depth_masking(
            ...
            depth_values=combined_valid_mask.float(),
            depth_mask_threshold_num=[1] * BS,
            valid_depth_range=(0.5, 1.5)
        )
    ...
    def forward(
        ...
        x, (B, S) = self.prepare_tokens_with_masks(
            x_img,
            x_depth,
            x_depth_coarse,
            x_depth_mask=x_depth_mask,
            x_depth_coarse_mask=x_depth_coarse_mask,
        )
