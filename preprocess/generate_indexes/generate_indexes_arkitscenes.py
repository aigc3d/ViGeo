...
def main():
    ...
    for split in ['Training', 'Test']:
        ...
        for scene in tqdm(scenes):
            ...
            depth_files = [os.path.join(
                split, scene, 'lowres_depth', file + '.png') for file in files]
            calib_file = os.path.join(split, scene, 'new_scene_metadata.npz')
    ...
    print(f"arkitscenes has {len(data)} scenes")
    with open(os.path.join(output_path, 'arkitscenes_train.json'), "w", encoding="utf-8") as f:
